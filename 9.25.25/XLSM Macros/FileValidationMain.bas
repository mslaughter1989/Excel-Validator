...
Public Function GetValidationRule(colRules As Collection, sFieldType As String, fieldName As String) As ValidationRule
...
Private Sub ValidateField(vFieldValue As Variant, sFieldType As String, lRowNumber As Long, colRules As Collection, oResult As ValidationResult)
    ...
    ' Record that we're checking this field
    ...
    Dim oRule As ValidationRule
    oRule = GetValidationRule(colRules, sFieldType, fieldName)
...
Private Sub FillDetailedSheet(ws As Worksheet, oResult As ValidationResult, sFilePath As String)
    ...
    With ws
        ...
        ' ADD VALIDATION CHECKS LOG SECTION:
        lRow = lRow + 2
        .Range("A" & lRow).Value = "VALIDATION CHECKS PERFORMED"
        .Range("A" & lRow).Font.Bold = True
        .Range("A" & lRow).Interior.Color = RGB(220, 220, 220)
        .Range("A" & lRow & ":C" & lRow).Merge
        
        lRow = lRow + 1
        For i = 1 To oResult.ValidationChecks.count
            .Range("A" & lRow).Value = "PASS"
            .Range("A" & lRow).Font.Color = RGB(0, 128, 0)
            .Range("B" & lRow).Value = oResult.ValidationChecks(i)
            lRow = lRow + 1
        Next i
