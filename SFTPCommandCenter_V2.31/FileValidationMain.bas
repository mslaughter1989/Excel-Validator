...
End If
        ...
        ' Validate GroupID matches expected
...
Private Sub ValidateRowFields(vData As Variant, lRow As Long, oMapping As ColumnMapping, colRules As Collection, oResult As ValidationResult)
    ...
    Debug.Print ">>> EXECUTING: FileValidationMain.ValidateRowFields at row " & lRow
...
Private Sub ValidateField(vFieldValue As Variant, sFieldType As String, lRowNumber As Long, colRules As Collection, oResult As ValidationResult)
    ...
    ' ========================================
    ' Define which fields should ALWAYS be checked for blanks
    ' Easy to add more fields here in the future!
    ...
    Select Case sFieldType
        ...
        ' Add new required fields here in the future:
        ' Case "NewFieldName"
        '     isRequiredField = True
        Case Else
...
Private Function CreateValidationReport(selectedFiles As Collection) As Workbook
    ...
    For Each sFilePath In selectedFiles
        ...
        ' Track if any files failed  <-- ADD THIS
        ...
        ' COLOR THE DETAIL SHEET TAB  <-- ADD THIS SECTION
        If oResult.IsValid Then
    ...
    ' COLOR THE SUMMARY TAB BASED ON OVERALL RESULTS  <-- ADD THIS SECTION
...
End Function
' ADD THIS FUNCTION TO YOUR FILEVALIDATIONMAIN MODULE:
